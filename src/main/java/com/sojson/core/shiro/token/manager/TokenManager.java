...
public class TokenManager {
    ...
    /**
     * 清空当前用户权限信息。 目的：为了在判断权限的时候，再次会再次 <code>doGetAuthorizationInfo(...)  </code>方法。 ps： 当然你可以手动调用 <code> doGetAuthorizationInfo(...)  </code>方法。
    ...
    public static void clearNowUserAuth() {
         ...
         * 获取到的时候是一个集合。Collection<Realm> RealmSecurityManager securityManager = (RealmSecurityManager) SecurityUtils.getSecurityManager(); SampleRealm realm =
