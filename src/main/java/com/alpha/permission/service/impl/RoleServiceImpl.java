...
public class RoleServiceImpl extends BaseMybatisDao<RoleMapper> implements RoleService {
	...
	public Pagination<RolePermissionAllocationBo> findRoleAndPermissionPage(Map<String, Object> resultMap,
		...
		Pagination<RolePermissionAllocationBo> page = super.findPage("findRoleAndPermission", "findCount", resultMap,
				pageNo, pageSize);
		if (page == null || CollectionUtils.isEmpty(page.getList())) {
			return null;
		}
		List<RolePermissionAllocationBo> boList = page.getList();
		List<RolePermissionAllocationBo> resList = Lists.newArrayList();
		for (RolePermissionAllocationBo bo : boList) {
			RolePermissionAllocationBo existsBo = findBo(resList, bo.getId());
			if (existsBo == null) {
				resList.add(bo);
			} else {
				existsBo.setPermissionIds(existsBo.getPermissionIds() + "," + bo.getPermissionIds());
				existsBo.setPermissionNames(existsBo.getPermissionNames() + "," + bo.getPermissionNames());
			}
		}
	...
	private RolePermissionAllocationBo findBo(List<RolePermissionAllocationBo> boList, Long id) {
