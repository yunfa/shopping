package com.alpha.core.freemarker;
...
public class Ferrmarker {
	...
	static {
		...
		/**
		 * 获取配置文件里的 Configuration.settings 设到当前Cfg里 Properties setting = new Properties(); Map<String,String> sets = vcfg.getSettings();
		 * setting.putAll(sets); try { cfg.setSettings(setting); } catch (TemplateException e) { logger.error("添加settings error;" ,e); }
		 */
	}
