...
public interface DsUserMapper {
	...
	DsUserBean getUserByName(@Param("userName") String userName, @Param("dbName") String dbName);

	int updateLoginPwd(DsUserBean bean);

	int updateAmount(DsUserBean bean);

	String getMobileByUserId(@Param("userId") Integer userId, @Param("dbName") String dbName);
}
