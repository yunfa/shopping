...
public class DateUtil {
	...
	public static Date[] rangeTimes(Date dateParam, int offset, int duration) {
		...
		if (offset != 0)
			date = DateUtil.add(date, Calendar.DATE, offset);
		date = DateUtil.truncate(date);
		dateTimes[0] = date;
		if (duration > 1)
			date = DateUtil.add(date, Calendar.DATE, duration);
		date = DateUtil.finalTime(date);
		dateTimes[1] = date;
		return dateTimes;
	}
	...
	public static String getOrderId(Date date) {
		return formatString(date, "yyyyMMddHHmmssSSS");
	}

	public static void main(String[] args) throws Exception {
		System.out.println(getOrderId(new Date()));
	}
}
